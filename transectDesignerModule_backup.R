...
plot_transect_preview <- function(bb_ll, terre, pt_landmark_ll, tran_ll,
  ...
  # Crop terre to bbox for faster plotting, but show full outline faintly if requested
  terre_crop_ll <- st_crop(terre_ll, st_bbox(bb_poly))
  
  p <- ggplot()
  
  if (show_full_terre) {
    p <- p + geom_sf(data = terre_ll, fill = "grey97", color = "grey85", linewidth = 0.15)
  } else {
    p <- p + geom_sf(data = terre_crop_ll, fill = "grey97", color = "grey85", linewidth = 0.15)
  }
  ...
  p <- p +
    ...
    geom_sf_text(data = tran_ll, aes(label = id), color = "red4", size = 3, check_overlap = TRUE) +
    { if (!is.null(pt_landmark_ll)) geom_sf(data = st_transform(pt_landmark_ll, 4326), color = "blue", size = 3) } +
    coord_sf(xlim = c(bb_ll["xmin"], bb_ll["xmax"]),
             ylim = c(bb_ll["ymin"], bb_ll["ymax"]),
             expand = FALSE) +
    theme_minimal()
  
  # Optional overlay points (already in terre_crs usually; transform to 4326)
  if (!is.null(air_sf) && nrow(air_sf) > 0) {
    p <- p + geom_sf(data = st_transform(air_sf, 4326), color = "blue", size = 1.2)
  }
  if (!is.null(muni_sf) && nrow(muni_sf) > 0) {
    p <- p + geom_sf(data = st_transform(muni_sf, 4326), color = "red", size = 1.2)
  }
...
refine_transects <- function(bb_ll, terre, pt_landmark_ll,
  ...
  repeat {
    ...
    cat("  n/s/e/w [km]               -> move anchor by km (default = spacing)\n")
